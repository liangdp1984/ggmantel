...
#' @param mantel_df mantel test data.frame, now it just support the data.frame returned by `fortify_mantel()`.
#' @param corr_df correalation test data.frame, now it just support the data.frame returned by `fortify_corr()`.
#' @param type correalation matrix plot style, should be "upper" or "lower".
...
ggmantel <- function(
                     ...
                     ylim = NULL)
{
  if(!inherits(mantel_df, "mantel_link_df"))
    stop("`mantel_df` need a 'mantel_link_df'.", call. = FALSE)
  if(!inherits(corr_df, "ggcorrr_df"))
    stop("`corr_df` need a 'ggcorrr_df'.", call. = FALSE)
  type <- match.arg(type)
  ...
  n <- length(unique(mantel_df$gdis))
  ...
  grp_name_df <- mantel_df[!duplicated(mantel_df$gdis), , drop = FALSE]
  mat_name_df <- mantel_df[!duplicated(mantel_df$mdis), , drop = FALSE]

  p <- ggplot() +
    geom_curve(aes(x = grp_x, y = grp_y, xend = mat_x, yend = mat_y,
                   size = `Mantel's r`, colour = `P values`), mantel_df,
               curvature = curvature) +
    geom_text(aes(x = grp_x + grp_label_hspace, y = grp_y + grp_label_vspace, label = gdis ),
